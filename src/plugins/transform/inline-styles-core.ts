...
// Core DOM+CSS logic for turning <style> rules into inline styles, shared by the
// `transform.style-inliner` module and the cohost site target (which needs the same
// "inline everything, cohost strips <style>" behaviour).

export type StyleInlinerMode = 'attr' | 'element';

const STYLES_TO_ATTR_VERSION = 2;
const SAFE_CONTAINER_TAGS = ['div', 'span', 'a', 'ul', 'ol', 'li', 'math', 'svg', 'g'];

export interface StyleInlinerStats {
...
/** Collapses every `<style>` rule into `style=""` attributes on the elements they match. */
export function stylesToAttrs(doc: Document, stats: StyleInlinerStats) {
    const styles = [];
...
/** Merges every `<style>` rule into a single `<style>` element appended to the body. */
export function stylesToBody(doc: Document, stats: StyleInlinerStats) {
    const styles = [];
    for (const style of doc.querySelectorAll('style')) {
        styles.push(style);
        style.remove();
